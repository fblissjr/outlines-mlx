...
class TransformerTokenizer(Tokenizer):
    ...
    def __init__(self, tokenizer):
        self.tokenizer = tokenizer
        self.eos_token_id = self.tokenizer._tokenizer.eos_token_id
        self.eos_token = self.tokenizer._tokenizer.eos_token

        if not self.tokenizer._tokenizer.pad_token_id:
            self.tokenizer._tokenizer.pad_token_id = (
                self.tokenizer._tokenizer.eos_token_id
            )
            self.pad_token_id = self.eos_token_id
        else:
            self.pad_token_id = self.tokenizer._tokenizer.pad_token_id
            self.pad_token = self.tokenizer._tokenizer.pad_token

        self.special_tokens = set(self.tokenizer._tokenizer.all_special_tokens)

        self.vocabulary = self.tokenizer._tokenizer.get_vocab()
        self.is_llama = isinstance(
            self.tokenizer._tokenizer, get_llama_tokenizer_types()
        )
...
def mlx(
    model_name: str, tokenizer_kwargs: dict = {}, adapter_file: Optional[str] = None
):
